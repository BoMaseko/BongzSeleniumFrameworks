...
import org.bongz.mail.SendEmailWithAttachements;

import static org.bongz.constants.FrameworkConstants.*;
import static org.bongz.mail.EmailConfig.*;

import org.bongz.constants.FrameworkConstants;

public class SendEmailUtils {

	public static void sendMail(int passTest, int failTest, int skipTest, int totalTest) throws Exception {
		
		if(PropertyUtils.getPropertyValue(ConfigProperties.SENDEMAILREPORT).equalsIgnoreCase("yes")) {
			...
			String messageBody = getTestCasesCountInFormat(passTest, failTest, skipTest, totalTest);
			System.out.println(messageBody);
			
			String attachmentFile_ExtentReport = FrameworkConstants.getExtentReportFilePath();
			
			SendEmailWithAttachements.sendEmailWithAttachments(SERVER, PORT, FROM, PASSWORD, TO, SUBJECT, messageBody, attachmentFile_ExtentReport);
